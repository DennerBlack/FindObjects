...
```
## Зависимости
Для начала необходимо, чтобы на компьютере были установлены Python 3.11, CUDA и CUDNN. Установить требуемые модули python можно, выполнив команду ниже:
```
...
Тестирование проводилось при следующих параметрах системы:
...
Необходима видеокарта Nvidia с памятью не меньше 4гб, поколения 10 или выше
## Детектор
### Обучение модели
Основные параметры модели, их можно менять в коде для создания собственной модели
```
train_dif - отвечает за сложность контрольного изображения, то есть данное изображение и его маска были убраны из оригинального датасета, чтобы модель не могла обучиться на нем и была возможность оценки качества обучения.
...
В тесте можно получить данный результат, передав image_path в cv2.imread: 
```
...
Изображение, размеченное детектором, но на нем ничего нет, как и должно быть)
<div align="center">
...
Исходное изображение - это валидационная картинка с не таким большим количеством шумов, разрешение 512х512. 
В тесте можно получить данный результат, передав image_path_norm в cv2.imread: 
```
...
В тесте можно получить данный результат, передав image_path_hard в cv2.imread: 
```
...
X=101, Y=287
```
В данном примере точность куда хуже, чем в предыдущем. Это связано с тем, что размер датасета слишком мал. При увеличении обучающей выборки точность возрастет.
